...
각 층의 주기는 아래로 갈수록 빨라지고, MPC 층의 주기는 정확히 "QP가 풀리는 속도"다(§3). 48강의 Atlas LBM이 "MPC급 전신 제어 위에" 학습 정책을 얹는다고 했던 그 아래층이 이 그림의 MPC+WBC이고, 24강에서 WBC를 채우면 이 스택이 완성된다.
...
— **뒷받침**: 18강에서 이어지는 상태공간·LQR·최적 제어 배경(E1~E2의 전제), §3의 지연·주기 논의의 기초(17강·18강과 동일 출처).
