...
> **참고**: `부록 E`가 언급하는 내부 문서(`ref/book.pdf`)는 팀 동료의 미공개 자료로 **이 저장소에 포함되지 않습니다**. 커리큘럼 구조에 착안만 얻었고, 모든 내용은 1차 자료(논문·공식 문서)로 교차검증하여 자체 집필했습니다.
...
5. **내부 문서(`ref/` 폴더 등)는 인용 출처로 쓰지 않는다** — 구조·아이디어 참고는 가능하되, 내용·수치는 반드시 1차 자료로 크로스체크한 뒤 그 1차 자료를 인용한다 (부록 E 참조).
...
| # | 제목 | 핵심 내용 | 실습 |
|---|---|---|---|
| [00](00-common/lec00-system-anatomy.md) | Physical AI 시스템 해부: 개념 좌표계 | 물리 플랜트 → 동역학 → 제어기 → 행동 명령 → 정책 → 학습 목적 → 표현 → 지각의 **계층 구분**을 커리큘럼 공용 좌표계로 도입. "VLA는 정책이고 컨트롤러는 정책이 아니다", "RL은 아키텍처가 아니라 학습 목적이다" 같은 **범주 구분** — 이후 모든 강의가 이 좌표를 참조. 두 트랙 수강생이 같은 지도를 공유하는 것이 목적 | 아는 시스템 하나(자율주행, 로봇셀, 드론)를 계층으로 분해하고 각 층의 입출력 신호를 명시해 보기 |

*이 강의의 계층 구분 프레임은 `ref/book.pdf`(내부 문서)의 구조에서 착안하되, 내용은 1차 자료(논문·공식 문서)로 크로스체크하여 자체 집필한다 (부록 E).*
...
## 부록 E — 내부 문서 `ref/book.pdf` 대응 분석

팀 동료가 작성한 "Foundations and Structure of Physical AI"(2026, 20장). **검증되지 않은 내부 문서이므로 교재·인용 출처로 쓰지 않는다** (인용 규약 5항). 대신 그 **구조적 통찰**을 분석해 커리큘럼에 다음과 같이 반영했다 — 반영된 내용은 전부 1차 자료로 크로스체크하여 자체 집필한다:

| book의 구조적 통찰 | 우리 커리큘럼의 대응 |
|---|---|
| 계층 좌표계(지각~플랜트의 층위 구분)로 개념 혼동을 정리 | **공용 00강 신설** — 좌표계를 자체 정리해 도입, 32강의 층위 진단으로 연결 |
| "로봇 배경 ↔ AI 배경" 양방향 독자 경로 | **하위제어 트랙(R01~R30) 신설** — 딥러닝 배경자를 위한 고전 로봇 30강 |
| 장 구조: 문제의식 → 직관 → 수식 3단(직관/의미/형식) → worked example(손계산+코드) → 흔한 오해 | **템플릿 v3에 채택**: "왜 이 강의가 필요한가", "핵심 수식(3단)", "Worked Example", "흔한 오해" 섹션 신설 |
| detection/SAM/affordance 등 우리가 건너뛴 CV 지형 | 4강 범위 확장 |
| SSL 계보(contrastive→JEPA→world model)의 체계적 취급 | 11강에 SSL 조감 추가, 31강에서 원논문 기준으로 심화 |
| VLA vs RL 류 논쟁의 직교 분해(아키텍처⊥학습목적) | 32강 논문 읽기 프레임워크에 "층위 진단" 도구로 흡수 |

책과 겹치지만 우리가 유지하는 차별점: 모델 계보의 역사(Part 5), 생태계·벤치마크(Part 7), 하드웨어 스펙(25강), 비공개 기업(24강), 실배포 수치(26강), 실습 중심 설계. 책을 읽고 싶은 팀원에게는 "개념 정리용 참고, 수치·주장은 원문 확인" 안내.
