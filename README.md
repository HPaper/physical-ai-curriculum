...
> 로봇 시스템을 **감지 → 판단 → 제어 → 구동 → 로봇 → 환경**의 닫힌 루프로 보고, 그 각 부분을 아래에서 위로 쌓아 올라가는 단일 학습 흐름(67강).
...
>
> **학습·개선 워크플로우**: 진도는 [`STUDY_LOG.md`](STUDY_LOG.md), Claude 세션 규약·강의 파일 규약은 [`CLAUDE.md`](CLAUDE.md). 새 세션에서 `/study-start`로 이어가고 `/study-end`로 마무리한다.

---

## ✅ 진행 상황 — 1차 전 과정(0–66강) 완성

이 저장소는 **0–66강 전 67강의 1차 집필을 완료**했습니다 — 전 강의 v3(핵심 수식 3단·실행 검증 코드·그림 ≥5). AI(Claude)와 함께 만든 살아있는 자료라 프론티어(Part 15)·모델 계보는 계속 갱신됩니다. (v3 = 수식 3단·실행 검증 코드·그림 ≥5)
...
- **전 과정(0–66강) 학습 가능**: 로봇의 몸·물리·구동·제어(1–24) → 딥러닝·Transformer·VLM·행동학습(25–41) → VLA 계보·실물 통합(42–50) → 환경·데이터·평가·시스템 통합·프론티어(51–66). 모든 강의 코드·수치·인용 검증 완료.
...
part09-robot-learning/         37–41 모방·생성·RL          ✅ 작성완료
...
part15-frontier/               63–66 프론티어·논문읽기·캡스톤·WM심화 ✅ 작성완료
...
- **한 강의 = 하루 6시간 모듈**: 이론 3~4h(수식 3단 + worked example + 코드) / 실습 1.5~2h / 토론·자가 점검 ~1h. 전체 67강 ≈ 하루 6시간·주 5일 기준 약 3개월.
...
```
   ...
   Part 2 기구학(1–8) → Part 3 동역학(9–13) → Part 4 구동(14–16) → Part 5 제어(17–24)
        ...
        → Part 13 데이터·평가(55–57) → Part 14 시스템 통합(58–62) → Part 15 프론티어(63–66)
```
...
| # | 제목 | 핵심 내용 | 실습 |
|---|---|---|---|
| [37](part09-robot-learning/lec37-imitation-learning-failure.md) | 모방학습이 무너지는 방식 | BC=지도학습, compounding error O(εT²), DAgger, causal confusion. **비유: 개루프 드리프트** | CS285 Lec 2 + BC+DAgger 축소판 |
| [38](part09-robot-learning/lec38-act-action-chunking.md) | ACT와 action chunking | ALOHA, CVAE, temporal ensembling | LeRobot ACT를 PushT에서 훈련 |
| [39](part09-robot-learning/lec39-diffusion-policy.md) | Diffusion Policy | 행동 다봉성, DDPM/DDIM 최소 수학, receding horizon. **비유: MPC의 확률적 사촌(23강)** | Diffusion Policy PushT |
| [40](part09-robot-learning/lec40-flow-matching.md) | Flow matching | 직선 보간 + ODE(4~10스텝). **π0/GR00T/SmolVLA 액션 헤드 공통 선택** | MIT 6.S184 flow matching from scratch |
| [41](part09-robot-learning/lec41-rl-crash-course.md) ★ | 강화학습 압축 코스 | MDP·정책경사·**advantage**·오프라인 RL·POMDP — 45강 RECAP을 읽기 위한 최소. **비유: 최적제어/HJB** (심화: 부록 D) | HF Deep RL PPO 또는 CS285 HW2 |
...
| # | 제목 | 핵심 내용 | 실습 |
|---|---|---|---|
| [63](part15-frontier/lec63-frontier-map.md) | 프론티어 지도 (2025-26) | ① RL post-training 물결 ② latent action(LAPA·GO-1) ③ **world model 수렴(V-JEPA 2·Cosmos·DreamZero: "VLA 다음은 WAM?")** ④ 효율화. **System 2/1/0 경계 엄밀 논의(GR00T vs Helix)** | 서베이 "An Anatomy of VLA Models" |
| [64](part15-frontier/lec64-reading-papers-framework.md) | 논문 읽기 프레임워크 | **6축 지도**(액션 디코딩/아키텍처/학습 레시피/데이터/평가/효율) + 비판적 읽기 체크리스트 10항목 + **층위 진단**(0강 지도로 "아키텍처⊥학습목적" 판별) | 체크리스트로 π0.7 논문 분석 |
| [65](part15-frontier/lec65-capstone.md) ★ | 캡스톤 | 최근 논문 2편을 프레임워크로 완전 분석 → "새로운 점"만 요약. 정보 파이프라인 구축 | 논문 분석 리포트 2편 |
| [66](part15-frontier/lec66-world-models-physical-ai.md) | World Model 기반 Physical AI 심화 (2026) | 54강 심화 특강: WM이 파이프라인에 꽂히는 **네 자리**(데이터 엔진/평가자/**플래너=학습된 모델 위 MPC(23강)**/정책=WAM). V-JEPA 2-AC·Cosmos·DreamGen·1X WM·**DreamZero**. 환각·평가 편향 한계 | WM 위 CEM 플래닝 numpy 재현 |
