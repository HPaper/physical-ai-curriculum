...
![WE-1 정적 역방향 pass 도해](../images/lec11/fig4_static_backward.png)

위 그림은 이 손계산을 그대로 시각화한 것이다(gen_figs.py로 생성). 왼쪽은 수평으로 뻗은 팔에서 역방향 pass가 말단→기저로 관성력 $f_i$와 관절 토크 $\tau_i$를 누적하는 모습 — 자식이 넘긴 $f_2$가 부모의 $f_1$에 더해진다. 오른쪽은 E2 역방향 식 $\tau_i = I_{zz,i}\alpha_i + c_i\times m_i a_{c,i} + \tau_{i+1} + p_{i+1}\times R f_{i+1}$의 정적 극한에서 $\tau_1$이 본문의 **세 조각**(자기 질량 팔길이 $19.62$ + 자식 모멘트 $n_2{=}\tau_2{=}5.886$ + 자식 힘 팔길이 $9.81$)으로 쌓이는 워터폴이고, 합 $35.316$이 폐형식 중력 항 $g_1$과 정확히 일치함을 함께 표시했다.
