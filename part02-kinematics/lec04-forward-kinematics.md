...
강체 변환은 6자유도인데 파라미터가 4개뿐인 것이 규약의 요체이자 함정이다: 프레임을 관절축 위에 강제로 올려서 2자유도를 소거했기 때문에, **프레임 위치가 링크의 물리적 위치와 무관해지고**(원점이 로봇 밖 허공에 놓이기도 한다), 인접 축이 평행하면 공통 수선이 유일하지 않아 파라미터 선택이 퇴화한다. 게다가 같은 이름으로 두 유파(standard와 Craig의 modified — 3강 §5에서 경고한 그 함정)가 공존해 표만 봐서는 규약을 확정할 수 없다 — 산업 문서를 읽을 때 반드시 먼저 확인할 것.
