...
![J의 SVD 파이프라인: 단위 관절속도 원이 조작성 타원으로](../images/lec06/fig4_svd_pipeline.png)

*E1의 "회전 → 축별 늘이기 → 회전"을 그대로 펼친 그림. 단위 관절속도 원 $\{\|\dot q\|=1\}$이 ① $V^\top$로 주입력방향에 정렬되고 → ② $\Sigma$로 축별로 $\sigma_i$배 늘어나 축정렬 타원이 되고 → ③ $U$로 회전해 EEF 속도 공간의 조작성 타원 $v^\top(JJ^\top)^{-1}v=1$이 된다. 반축은 $\sigma_i u_i$. 위=건강한 자세($q_2{=}90°$, $\sigma{=}(1.62,0.62)$, WE-1의 그 값)의 통통한 타원, 아래=특이점 근처($q_2{=}8°$)에서 $\sigma_2$가 작아져 타원이 바늘로 붕괴 — 짧은 반축 방향 $u_2$가 바로 WE-1의 "잃어가는 radial 방향"이고, 관절 쪽 $v_2$는 폭증이 타고 흐르는 null 방향이다. $l_1{=}l_2{=}1$, `gen_figs.py`로 생성.*
...
⬅ 이전: [Lec 05. 자코비안 — 속도의 사상, 힘의 쌍대](lec05-jacobian.md)　｜　[📖 전체 목차](../README.md)　｜　다음: [Lec 07. 역기구학 — 해석해, 수치해, 그리고 다해성](lec07-inverse-kinematics.md) ➡
