...
초기값 공간이 두 해에 거의 반반(44.0% vs 45.2%)으로 갈리고, 10.9%는 수렴에 실패한다. $q_2(0)$의 부호만 바꾼 두 초기값이 서로 다른 해로 간다 — WE-1의 두 해석해가 각각의 어트랙터임을 수치해 쪽에서 재확인한 것이다. §3의 basin 지도(그림 3)가 이 코드의 고해상도 버전이다(`images/lec07/gen_figs.py`).
...
4. **(20분) null-space 과제.** 1강 WE-2의 3R 팔($L = [0.6, 0.5, 0.4]$)에서 EEF를 고정한 채 $z = 0.1\,(q_{\text{pref}} - q)$, $q_{\text{pref}} = (0, 0.9, 0)$을 null-space로 흘려 넣어라 (§4 식). 200회 반복 후 관절은 눈에 띄게 재배치되지만 EEF 이동량이 $10^{-15}$ 이하임을 확인하라 — self-motion manifold 위를 미끄러진 것이다.
