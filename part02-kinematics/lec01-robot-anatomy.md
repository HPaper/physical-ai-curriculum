# Lec 01. 로봇 해부학 — 링크, 관절, 그리고 자유도

> 하위제어 트랙 1일차. 선수 지식: 공용 0강.
> 기초 참고서: Modern Robotics(이하 MR) Ch.2. 이 강의는 MR §2.1~2.5의 내용을 딥러닝 배경자의 언어로 재구성한 것이다.

## 한 장 요약

![2R 작업공간과 3R 여유자유도](../images/lec01/fig1_workspace_redundancy.png)
...
그 전에, 오늘 다룰 두 가지 기계 구조의 골격을 한눈에 잡아 두자. 로봇은 링크를 관절로 이은 그래프다. 이 그래프에 **닫힌 루프가 없으면 직렬(개루프)**, **있으면 병렬(폐루프)** — 이 위상 차이가 곧 Grübler 공식에서 자유도가 갈리는 이유다(§3 · WE-1, 아래 fig2·fig3).

```mermaid
graph TD
    subgraph S["직렬 (개루프 사슬): dof = 관절 합"]
        G1["지면(base)"] -->|R1| A1["링크 L1"]
        A1 -->|R2| A2["링크 L2"]
        A2 -->|R3| A3["링크 L3 · EEF"]
    end
    subgraph P["병렬 (폐루프): 루프가 닫혀 dof < 관절 합"]
        G2["지면(base)"] -->|R1| B1["다리1 상단"]
        G2 -->|R5| B2["다리2 상단"]
        B1 -->|R2| C1["플랫폼(EEF)"]
        B2 -->|R4| C1
    end
    style A3 fill:#2ca02c,color:#fff
    style C1 fill:#2ca02c,color:#fff
    style G1 fill:#eee
    style G2 fill:#eee
```

트리(직렬)에서는 지면부터 EEF까지 경로가 하나뿐이라 관절 구속이 겹치지 않아 dof = Σfᵢ지만, 플랫폼으로 두 경로가 모여드는 병렬에서는 루프가 닫히며 구속이 서로 얽혀 dof가 깎인다.
...
딥러닝에서 온 사람에게 로봇의 첫 인상은 "출력 차원이 관절 수인 모델"이다. 그러나 AI·VLA 파트 50강에서 봤듯 VLA의 action space는 7차원(ΔEEF)이기도, 14차원(양팔 관절)이기도, 132차원(GR00T N1.7)이기도 하다. 그 숫자들이 어디서 오는가? 로봇의 **기계적 구조**에서 온다. DoF를 세지 못하면 action space를 설계할 수도, 논문의 "6-DoF 태스크" 같은 표현을 읽을 수도, 왜 휴머노이드 손이 22-DoF인데 액추에이터는 더 적은지(부족구동, 13강)도 이해할 수 없다. 오늘은 로봇이라는 기계의 "형(type)"을 배운다 — 움직임의 수학(2강~)은 그 다음이다.
...
주의할 점: C-space는 유클리드 공간이 아니다. 회전 관절 하나의 구성은 원 $S^1$ 위의 점이고(0°와 360°는 같은 구성), 2R 팔의 C-space는 토러스 $T^2 = S^1 \times S^1$이다. "각도를 그냥 실수로 다루면" 랩어라운드에서 문제가 생기는 이유이며, 2강에서 회전의 표현 문제로 본격화된다.
...
**물리·기하적 의미**: 정기구학 $x = f(q)$는 C-space에서 작업공간으로 가는 **비선형 사상**이고, 도달 작업공간은 그 사상의 **상(image)**이다. 한 장 요약 (a)에서 관절공간을 균일하게 표본했는데 작업공간 밀도가 불균일했던 이유 — 사상이 공간을 접고 늘리기 때문(이 왜곡률이 5강의 자코비안이다).
...
![관절공간에서 작업공간으로의 사상](../images/lec01/fig4_joint_to_task.png)

2R 팔에서 사상 $x=f(q)$가 공간을 어떻게 접고 늘리는지 본 그림. 왼쪽은 관절공간 $\mathcal{C}=T^2$의 $q_1$=상수 격자선(색으로 구분), 오른쪽은 그 선들의 상(image)이 작업공간의 도달 고리(내경 $|l_1{-}l_2|$, 외경 $l_1{+}l_2$) 안에서 원호로 휘어 겹치는 모습이다 — 균일한 관절 표본이 왜 불균일한 작업공간 밀도를 만드는지(이 왜곡률이 5강의 자코비안)를 시각화(images/lec01/gen_figs.py로 생성).

![여유자유도의 1차원 해족과 DoF 예산](../images/lec01/fig5_redundancy_and_gripper.png)

왼쪽: 3R 팔로 같은 EEF **위치**(별)를 겨냥해 서로 다른 초기값에서 IK를 풀면, 전혀 다른 팔 자세들이 모두 목표를 맞춘다 — 여유자유도 $n{-}t=3{-}2=1$의 1차원 해족을 직접 표본한 것. 오른쪽: §3 표의 로봇들을 "태스크 차원 $t$(파랑) + 여유 $n{-}t$(초록) = 관절 수 $n$"으로 분해한 DoF 예산 — Franka의 +1이 어디서 오는지를 한눈에 보여준다(images/lec01/gen_figs.py로 생성).
...
![직렬 vs 병렬 구조의 자유도 비교](../images/lec01/fig2_serial_vs_parallel.png)

직렬(개루프 3R)과 병렬(폐루프 5절)을 나란히 두고 Grübler를 적용한 그림. 왼쪽은 지면→EEF 경로가 하나뿐이라 dof=관절 합(3), 오른쪽은 플랫폼으로 두 다리가 모여 루프가 닫히면서 관절 5개인데도 dof=2로 깎인다 — E2의 "관절은 자유를 깎는 장치"가 폐루프에서 어떻게 극적으로 나타나는지 보여준다(images/lec01/gen_figs.py로 생성).

![Grübler 공식 자유도 분해 도해](../images/lec01/fig3_grubler_breakdown.png)

같은 3종 메커니즘을 Grübler 식 $\text{dof}=m(N{-}1)-\sum_i(m-f_i)$의 "자유(파랑) − 구속(빨강) = 남는 dof(초록)" 막대로 분해한 그림. 4절→1, 병렬 5절→2, 공간 6R→6이 세 갈래 셈에서 그대로 떨어지는 것을 눈으로 확인할 수 있다 — E2 유도의 항별 의미를 시각화(images/lec01/gen_figs.py로 생성).
...
def fk(q):
    ...
    return p
def ik(q):                                   # damped least squares (7강에서 유도)
    for _ in range(300):
        e = target - fk(q)
        ...
        for j in range(3):
            dq = q.copy(); dq[j] += eps
            J[:, j] = (fk(dq) - fk(q))/eps   # 수치 자코비안 (5강 예고)
...
### 3. 실제 로봇의 DoF 읽기 — 숫자에는 이유가 있다

| 로봇 | DoF | 왜 그 숫자인가 |
|---|---|---|
| UR5e | 6 | 공간 강체 자세 지정에 필요한 최소치(위치 3+자세 3). 여유 없음 — 특이점 회피 여지도 없음(6강) |
...
| 2지 그리퍼 | 1 | pick-and-place에는 1이면 충분하다는 공학적 판단 — 49강의 "핸드의 분화" 참조 |

AI·VLA 파트과의 연결: VLA의 action 차원은 이 표에서 나온다. π0의 "최대 18차원"(50강)은 양팔 6+6, 그리퍼 1+1, 베이스 3, 토르소 1의 합이다. **action space 설계란 이 표의 어느 부분집합을 정책에 노출할지의 결정**이다.
...
- **여유자유도는 과매개변수화(over-parameterization)다** — 같은 출력(EEF 자세)을 내는 입력(관절각)의 연속족이 존재. 신경망에서 같은 함수를 구현하는 가중치가 다양체를 이루는 것과 구조가 같고, "그 여분을 무엇에 쓸 것인가"(정칙화 ↔ null-space 태스크, 6강)라는 질문도 평행하다.
...
3. **"6-DoF 팔이면 작업공간 안 어디든 어떤 자세로든 간다"** — 관절 한계, 자기충돌, 특이점(6강)이 실제 가용 영역을 크게 깎는다. 스펙시트의 "reach 855mm"는 그 어느 것도 반영하지 않은 숫자다.
4. **"여유자유도는 낭비다"** — 반대로, 7번째 관절은 특이점·장애물·관절한계를 피하는 **추가 예산**이다. 대가는 IK의 해가 유일하지 않아진다는 것(7강) — 그리고 그 비유일성이야말로 학습 정책이 다봉 분포를 다뤄야 하는(39강) 기구학적 뿌리 중 하나다.
...
3. (선택) 49강의 로봇 표를 다시 보며 각 로봇의 DoF 열을 오늘의 눈으로 재해석.
...
— **뒷받침**: "손가락 22-DoF, 부족구동/커플링" 사례 — 공식 스펙시트 부재, 49강과 동일한 유보.
