...
그 전에, 오늘 다룰 두 가지 기계 구조의 골격을 한눈에 잡아 두자. 로봇은 링크를 관절로 이은 그래프다. 이 그래프에 **닫힌 루프가 없으면 직렬(개루프)**, **있으면 병렬(폐루프)** — 이 위상 차이가 곧 Grübler 공식에서 자유도가 갈리는 이유다(§5 · WE-1, 아래 fig2·fig3).
...
왼쪽: 3R 팔로 같은 EEF **위치**(별)를 겨냥해 서로 다른 초기값에서 IK를 풀면, 전혀 다른 팔 자세들이 모두 목표를 맞춘다 — 여유자유도 $n{-}t=3{-}2=1$의 1차원 해족을 직접 표본한 것. 오른쪽: §5 표의 로봇들을 "태스크 차원 $t$(파랑) + 여유 $n{-}t$(초록) = 관절 수 $n$"으로 분해한 DoF 예산 — Franka의 +1이 어디서 오는지를 한눈에 보여준다(images/lec01/gen_figs.py로 생성).
