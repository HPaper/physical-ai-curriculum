...
1. **"SmolVLA와 SimVLA는 같은 모델(또는 후속판)이다"** — 아니다, 이름만 닮은 **별개 모델**이다. SmolVLA는 Hugging Face의 450M 온디바이스 VLA(2025.6, arXiv 2506.01844)로 커뮤니티 SO-100 LeRobot 데이터로 훈련된 π0 템플릿의 축소판이고, SimVLA는 그와 무관한 0.5B 미니멀 베이스라인(2026.2, arXiv 2602.18224)이다. 파라미터가 비슷하고(0.45B vs 0.5B) 둘 다 "flow 헤드 + 사전학습 VLM"을 쓴다는 점이 혼동을 부르지만, **출처·데이터·주장이 다르다** — SmolVLA는 커뮤니티 데이터 민주화가 메시지고, SimVLA는 "그 복잡성이 다 필요한가"라는 심 벤치마크 위 회의론이 메시지다(자가 점검 3의 이유).
2. **"작은 모델은 큰 모델보다 항상 열등하다"** — 크기는 한 축일 뿐이다. SmolVLA(450M)는 π0(3.3B) 대비 **약 15배 가벼운 배포**(추론 ~2GB vs ~14GB)로 노트북에 들어가고, LIBERO·Meta-World·SO-100 실기에서 훨씬 큰 모델들과 대등 이상을 보인다(E2). TinyVLA는 "작은 VLM + 제대로 된 연속 헤드 > 거대 AR VLA"의 증거다. "작다=졌다"가 아니라 **효율·온디바이스·특정 태스크에서의 다른 베팅**이며, 오히려 "정말 필요한 게 뭔가"의 ablation(과학적 동기)이 경량화의 세 동기 중 하나다.
3. **"TinyVLA는 SmolVLA의 축소판이다"** — 시간 순서부터 반대다. TinyVLA는 **2024.9로 SmolVLA(2025.6)보다 먼저** 나왔으니 축소판일 수 없다. 계보도(한 장 요약)에서 TinyVLA는 "경량+연속 헤드"를 SmolVLA로 **물려준 선행 탐험**이지 파생이 아니다. 게다가 백본 계통도 다르다 — SmolVLA는 SmolVLM-2(SigLIP+SmolLM2) 유래 백본에 π0식 flow expert를 얹은 반면, TinyVLA는 <1B VLM에 **디퓨전 정책 헤드**를 붙이고 로봇 사전학습을 통째로 생략(LoRA ~5%)한 설계다. 두 모델은 "경량화"라는 방향만 공유한다.
4. **"GO-1의 latent action은 사람이 라벨링한 행동을 압축한 것이다"** — 정반대다. latent action의 핵심은 **행동 라벨이 전혀 없는** 웹·인간 영상에서 "무엇이 어떻게 움직였나"를 스스로 역추정해 만든 잠재 행동이다(LAPA '24.10 계보). GO-1의 **ViLLA**는 VLM + Latent Planner + Action Expert의 **3단** 구조로, 그 latent action을 중간 계획 층으로 쓴다 — "VLA에 planner 하나 더 붙인 것"이 아니라 **무라벨 영상을 행동 데이터로 전용하는** 데이터 축의 탐험이다. 다만 원리적 한계가 있다: 영상은 운동학만 담아 힘·강성은 못 복원한다(E3의 null space).
5. **"RDT와 CogACT는 π0의 아류(같은 flow expert 계열)다"** — 둘 다 **의도적 대조군**이지 계승이 아니다. RDT-1B(2024.10)는 **VLM 백본이 아예 없는** 순수 디퓨전 트랜스포머 1.2B로 "VLM이 정말 필수인가"를 묻는 대조군이다. CogACT(2024.11)는 7B VLM(인지)과 ~300M DiT(행동)를 **명시적으로 분리**해, π0가 attention 공유로 "접은" 것을 모듈 분리로 "편" 반대 극단이다(44강의 "attention 공유 vs 완전 분리" 축의 실존 답안). 세 논쟁 축에서 이 둘은 각각 구조 축의 서로 다른 좌표를 찍는다 — 계보도에서 점선(대조·회의)으로 연결되는 이유다.
