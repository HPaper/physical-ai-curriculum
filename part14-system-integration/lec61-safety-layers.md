...
- **3층 — 소프트웨어 안전 필터.** 오늘의 주인공. 정책 출력과 제어기 사이에서 명령을 실시간으로 **최소 수정**한다(핵심 수식·Worked Example).
...
1. **필터 위치 결정**: 오늘은 필터를 **참조 속도 레벨**(정책과 제어기 사이, §3 다이어그램의 3층)에 놓는다. 참조점 $p_f$를 $\dot p_f = v_f$로 적분하되, $v_{des} = \dot p_{circle} + k_r(p_{circle} - p_f)$의 $x$ 성분에만 E3의 해석해를 적용한다:
...
— **뒷받침**: §1·§3 — 2층(펌웨어 한계)의 실례: 토크·위치 지령에 대한 가속도·저크·토크레이트 리미터, 한계 위반 시 reflex 정지 (50강·19강와 동일 출처).
