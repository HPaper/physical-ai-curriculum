# Lec 24. 비공개 진영의 지향점 — 회사별 베팅으로 읽는 VLA 설계 철학

> Part 5 마지막 강의. 선수 지식: 18~23강 (특히 22강 GR00T의 dual-system, 21강 RECAP).
...
**베팅 요약**: 21강 RECAP과 대비하라 — PI는 "실기 데이터 + RL로 신뢰성", Skild는 "심 스케일 + 적응력". 데이터 철학의 양극단.
...
- 가장 실질적인 공개는 **신경 world simulator**(Elluswamy, ICCV 2025): 행동 조건부 비디오 생성 모델을 FSD와 Optimus 양쪽의 훈련·평가에 사용. 31강(world model)의 예고편.
...
- **TRI LBM 논문**(2025.7, arXiv 2507.05331): 언어+비전+고유수용감각 조건부 디퓨전 트랜스포머, flow matching 목적함수, 16-스텝 액션 청크, ~1,700시간 로봇 데이터. 핵심 발견: **멀티태스크 사전학습이 새 태스크에서 3~5배 데이터 효율** + 통계적으로 엄밀한 평가 방법론(30강과 연결).
...
**베팅 요약**: 계층화를 한 모델 안(π0.5)이 아니라 **두 모델 사이**에 둔 형태. 24강 지도에서 Figure와 같은 사분면, 다른 구현.
...
3. **world model의 침투**: Tesla 신경 시뮬레이터(훈련·평가), 1X WM(실행 전 평가), NVIDIA Cosmos/DreamZero(합성 데이터·zero-shot 정책). 31강의 주제가 이미 상용에 들어와 있다.
...
1. Skild의 "VLM에 로봇 데이터 1% 접붙이기" 비판은 타당한가? π0.5의 co-training 결과(21강)는 이 비판에 대한 반박 근거가 되는가?
...
3. (선택) TRI LBM 프로젝트 페이지: 초록 + 평가 방법론 섹션만 — 30강에서 다시 만난다.
